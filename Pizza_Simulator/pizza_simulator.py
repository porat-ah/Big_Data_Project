...
class branch:
    ...
    def get_order_time_length(self):
        return max(np.random.normal(self.average_order_time, self.std_order_time), 1)
...
class order:
    ...
    def order_to_json(self):
        if self.is_order_in_process():
            order_status = "in_process" 
            time = str(self.time)
        
        else:
            order_status = "finished"
            time = str(self.time + datetime.timedelta(minutes= self.order_time_length))
